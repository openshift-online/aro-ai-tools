...
# get-releases.sh — wraps the ARO HCP oncall dashboard CLI ('oncall builds')
# and filters the resulting pipeline builds down to a single environment.
#
...
usage() {
    cat >&2 <<'EOF'
get-releases.sh — List ARO HCP pipeline releases (ADO builds) for one environment

USAGE:
    get-releases.sh -Environment <env> [options]
...
EXAMPLES:
    get-releases.sh -Environment stg
    get-releases.sh -Environment test -Hours 6
    get-releases.sh -Environment prod -Hours 0
EOF
    exit 1
}
...
END {
    if (matched>0) {
        ...
        printf("No releases matched environment \"%s\".\n", env) > "/dev/stderr"
